...
st.markdown("""<style>
    ...
    .grid-card { background: #131B2E; border-radius: 10px; border: 1px solid #212B41; padding: 15px; margin-bottom: 15px; height: auto; }
...
if run_engine and API_KEY:
    ...
    if not df.empty:
        ...
        # 👥 2위 ~ 20위 도전자 레이아웃 (안전한 컴포넌트 맵핑으로 우회)
        ...
        for i in range(len(g_data)):
            ...
            with cols[i % 3]:
                st.markdown(f"""<div class="grid-card">
                    <div>
                        <div style="display:flex;justify-content:space-between;font-size:8pt;"><b>TOP {i+2}</b><span style="{tc}">{item['type']}</span></div>
